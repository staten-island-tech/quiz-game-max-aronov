...
var GENERATOR = {
  ...
  UnaryExpression: function UnaryExpression(node, state) {
    if (node.prefix) {
      ...
      if (operator.length > 1 || type[0] === 'U' && (type[1] === 'n' || type[1] === 'p') && argument.prefix && argument.operator[0] === operator && (operator === '+' || operator === '-')) {
        state.write(' ');
      }

      if (state.expressionsPrecedence[type] < state.expressionsPrecedence.UnaryExpression) {
        state.write('(');
  ...
  ConditionalExpression: function ConditionalExpression(node, state) {
    if (state.expressionsPrecedence[node.test.type] > state.expressionsPrecedence.ConditionalExpression) {
      this[node.test.type](node.test, state);
    } else {
      state.write('(');
      this[node.test.type](node.test, state);
  ...
  NewExpression: function NewExpression(node, state) {
    ...
    if (state.expressionsPrecedence[node.callee.type] < state.expressionsPrecedence.CallExpression || hasCallExpression(node.callee)) {
      state.write('(');
  ...
  CallExpression: function CallExpression(node, state) {
    if (state.expressionsPrecedence[node.callee.type] < state.expressionsPrecedence.CallExpression) {
      state.write('(');
      this[node.callee.type](node.callee, state);
      state.write(')');
    } else {
  ...
  MemberExpression: function MemberExpression(node, state) {
    if (state.expressionsPrecedence[node.object.type] < state.expressionsPrecedence.MemberExpression) {
      state.write('(');
      this[node.object.type](node.object, state);
      state.write(')');
    } else {
